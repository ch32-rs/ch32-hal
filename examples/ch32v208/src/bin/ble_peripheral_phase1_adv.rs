//! BLE ADV_IND ch37 PeripheralRole Phase 1 probe — CH32V208, task #68.
//!
//! This keeps the proven `ble_tx_adv_ch37` register path and swaps the PDU
//! payload to a connectable Phase 1 peripheral probe:
//!   - Access address: 0x8E89_BED6 (BLE ADV AA, not DTM 0x71764129)
//!   - Channel field: logical ch=37
//!   - PDU format: ADV_IND / SCAN_RSP header + AdvA(6B) + AD payload
...
//! # Validation (Cindy)
//!
//! Primary: phone/Mac scanner sees AdvA=C2:21:43:65:87:12 + "Simple".
//! Secondary: second CH32 running `ble_rx_listener` captures CONNECT_IND when
//! a central attempts to connect.
//!
                ...
                3 => {
                    ...
                    if blk & (1 << 6) != 0 {
                        ...
                        if ip4v & 0x40 == 0 {
                            ...
                            let _timer =
                                read_volatile((core::ptr::addr_of!(gBleIPPara) as *const u8).add(16) as *const u32);
                        }
                    }
                }
                4 => {
                    ...
                    if blk & (1 << 6) != 0 {
                        ...
                        if ip4v & 0x40 == 0 {
                            ...
                            let timer =
                                read_volatile((core::ptr::addr_of!(gBleIPPara) as *const u8).add(16) as *const u32);
                            write_volatile((BB_BASE + 0x64) as *mut u32, timer);
                            // WCH_LLER+0x64
                        }
                    }
                }
...
unsafe fn build_common_ad_payload(mut pos: usize) -> usize {
    ...
    // AD: Shortened Local Name "Simple" (type 0x08).
    let name = b"Simple";
...
/// Build ADV_IND PDU into TX_BUF.
///
/// AD payload: Flags(3B) + Manufacturer "ble"(5B) + Shortened Local Name
/// "Simple"(8B). Total payload = AdvA(6) + AD(16) = 22B.
unsafe fn build_adv_pdu() -> usize {
    ...
    pos = build_common_ad_payload(pos);

    let payload_len = pos - 2; // AdvA + AD, excludes header bytes

    // Header byte 0: ADV_IND (type=0x00), EVT-style TxAdd=0 by default.
    TX_BUF[0] = 0x00 | if TXADD_RANDOM { 1 << 6 } else { 0 };
...
/// Build SCAN_RSP PDU into TX_BUF.
///
/// Phase 1 sends this as a standalone compatibility probe. Precise
/// SCAN_REQ→SCAN_RSP turnaround belongs to the future connection/scan state
/// machine.
unsafe fn build_scan_rsp_pdu() -> usize {
    let mut pos = 2usize; // skip header bytes, fill below
    ...
    // Keep SCAN_RSP compact: UUID16 0xFFF0 + Complete Local Name.
    TX_BUF[pos] = 3; // length: type + UUID16
    TX_BUF[pos + 1] = 0x03; // Complete List of 16-bit Service UUIDs
    TX_BUF[pos + 2] = 0xF0;
    TX_BUF[pos + 3] = 0xFF;
    pos += 4;

    let name = b"Simple";
    TX_BUF[pos] = (name.len() + 1) as u8;
    TX_BUF[pos + 1] = 0x09; // Complete Local Name
    TX_BUF[pos + 2..pos + 2 + name.len()].copy_from_slice(name);
    pos += 2 + name.len();

    let payload_len = pos - 2; // AdvA + ScanRspData
    TX_BUF[0] = 0x04 | if TXADD_RANDOM { 1 << 6 } else { 0 };
    TX_BUF[1] = payload_len as u8 & 0x3F;
    ...
    match ISO_MODE {
        ...
        _ => {
            ...
            if !TRIGGER_POST_PRINTED {
                ...
                hal::println!(
                    "# TRIGGER_POST_SCAN t=0_pre:  bb08={:#010x} bb1c={:#010x}",
                    pre_bb08,
                    pre_bb1c
                );
                hal::println!(
                    "# TRIGGER_POST_SCAN t=+100us: bb08={:#010x} bb1c={:#010x}",
                    post_08_100,
                    post_1c_100
                );
                hal::println!(
                    "# TRIGGER_POST_SCAN t=+200us: bb08={:#010x} bb1c={:#010x}",
                    post_08_200,
                    post_1c_200
                );
                hal::println!(
                    "# TRIGGER_POST_SCAN t=+400us: bb08={:#010x} bb1c={:#010x}",
                    post_08_400,
                    post_1c_400
                );
                hal::println!(
                    "# TRIGGER_POST_SCAN t=+521us: bb08={:#010x} bb1c={:#010x}",
                    post_08_521,
                    post_1c_521
                );
                hal::println!(
                    "# X1_POLLED_W1C enabled={} bb_ack_count={} bb_last={:#010x} lle_ack_count={} lle_last={:#010x}",
                    X1_POLLED_W1C,
                    x1_bb_ack_count,
                    x1_last_bb_mask,
                    x1_lle_ack_count,
                ...
                hal::println!(
                    "# Y3_TRIGGER_ACK bb00_changed_iter={} bb00_val={:#010x} bb64_changed_iter={} bb64_val={:#010x}",
                    y3_bb00_changed_iter,
                    y3_bb00_changed_val,
                    y3_bb64_changed_iter,
                    y3_bb64_changed_val
                );
                hal::println!(
                    "# PATHC_MANUAL_L6 enabled={} hit={} wait={} before={:#010x} after={:#010x}",
                    PATHC_MANUAL_L6,
                    MANUAL_L6_HIT,
                    MANUAL_L6_WAIT,
                    MANUAL_L6_BEFORE,
                ...
                hal::println!(
                    "# PATHC_MANUAL_L6_IP4 before={:#04x} after={:#04x}",
                    MANUAL_L6_IP4_BEFORE,
                    MANUAL_L6_IP4_AFTER
                );
            ...
            if Z37_DUMPED && !Z37_PRINTED {
                Z37_PRINTED = true;
                hal::println!(
                    "# Z37 Rust poll-stop burst={} hit={} bb08={:#010x} timeout_left={}",
                    Z37_TARGET_BURST + 1,
                    Z37_HIT,
                    Z37_BB08,
                    Z37_TIMEOUT_LEFT
            ...
            if BB08_TRACE_DONE && !BB08_TRACE_PRINTED {
                ...
                hal::println!(
                    "# BB08_TRACE Rust burst={} samples=200 step_us≈1.25",
                    Z37_TARGET_BURST + 1
                );
                let mut last = 0u32;
                for j in 0..200 {
...
unsafe fn wait_adv_tx_before_rx_turnaround() -> bool {
    // Phase 2 probe: keep the ADV_IND→RX handoff close to T_IFS.  The normal
...
unsafe fn rx_turnaround_capture_ch37(adv_channel: u8) -> Option<[u8; 50]> {
    ...
    bb_write(0x74, addr_of!(RX_TURNAROUND_BUF) as u32);
...
unsafe fn log_connect_ind(tx_n: u32, snap: &[u8; 50]) -> bool {
    let pdu_type = snap[0] & 0x0F;
    let pdu_len = snap[1] & 0x3F;
    if pdu_type == 5 && pdu_len == 34 {
        let adv_match = snap[8..14] == ADDR;
        let access_addr = u32::from_le_bytes([snap[14], snap[15], snap[16], snap[17]]);
        let crc_init = (snap[18] as u32) | ((snap[19] as u32) << 8) | ((snap[20] as u32) << 16);
        let win_size = snap[21];
        let win_offset = u16::from_le_bytes([snap[22], snap[23]]);
        let interval = u16::from_le_bytes([snap[24], snap[25]]);
        let latency = u16::from_le_bytes([snap[26], snap[27]]);
        let timeout = u16::from_le_bytes([snap[28], snap[29]]);
        let hop_sca = snap[35];
        hal::println!(
            "# CONNECT_IND tx#{tx_n} InitA={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} \
             AdvA={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} adv_match={} \
             AA={:#010x} crc={:#08x} win={} off={} interval={} latency={} timeout={} \
             chm={:02x}{:02x}{:02x}{:02x}{:02x} hop={} sca={}",
            snap[7],
            snap[6],
            snap[5],
            snap[4],
...
#[inline(always)]
fn next_adv_delay_us(state: &mut u32) -> u32 {
...
unsafe fn inject_evt_padvctx() {
    ...
    hal::println!(
        "H_PADVCTX: INJ_START dst=0x{:08x} len={}",
        EVT_PADVCTX_ADDR,
        EVT_PADVCTX.len()
    );
...
async fn main(_spawner: embassy_executor::Spawner) -> ! {
    ...
    hal::println!(
        "AdvA: {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} (random static)",
        ADDR[5],
        ADDR[4],
        ADDR[3],
        ADDR[2],
    ...
    unsafe {
        ...
        // Step 2: Build TX PDUs once for diagnostics. The loop rebuilds before
        // each TX because ADV_IND and SCAN_RSP share the same DMA buffer.
        let adv_pdu_len = build_adv_pdu();
        hal::println!(
            "ADV_IND built: {}B total (header=2 + payload={})",
            adv_pdu_len,
            adv_pdu_len - 2
        );
        hal::println!(
            "  hdr=[{:#04x},{:#04x}] AdvA=[{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}]",
            TX_BUF[0],
            TX_BUF[1],
            TX_BUF[7],
            TX_BUF[6],
        ...
        let scan_rsp_len = build_scan_rsp_pdu();
        hal::println!(
            "SCAN_RSP built: {}B total (header=2 + payload={})",
            scan_rsp_len,
            scan_rsp_len - 2
        );
